...
class AppStore extends Component {
  state = {SearchInput: '', filterSearchInput: ''}

  filterAppsearchInput = event => {
    this.setState({SearchInput: event.target.value})
  }

  TabButtonFilter = () => {
    return console.log('hlw')
  }

  render() {
    const {SearchInput, filterSearchInput} = this.state
    ...
    return (
      <div className="bg-container">
        <hi className="heading">App Store</hi>
        <div className="container">
          <input
            ...
            placeholder="Search"
            value={SearchInput}
            onChange={this.filterAppsearchInput}
          />
        </div>

        <ul className="unorderlist">
          {tabsList.map(eachTab => (
            <TabItem key={eachTab.tabId} Tabdetails={eachTab.displayText} />
          ))}
        </ul>
        <ul className="un-list">
          {searchResult.map(EachApp => (
            <AppItem
              ...
              TabButtonFilter={this.TabButtonFilter}
            />
          ))}
        </ul>
      </div>
